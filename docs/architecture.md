...
`renderer/dom.ts` が生成HTMLと既存DOMを照合し、同じ入力コントロールを維持する。textareaの子テキストは通常通知では更新せず、下書き挿入・送信時にlive valueを明示更新する。設定の下書きも値が異なる場合のみ反映し、ブラウザーのUndo履歴を壊さない。ハンドラーはプロパティへの代入で更新し、多重登録を避ける。変換中の描画保留を維持し、同じ要素にフォーカスが残る場合は選択位置の再設定も行わない。通知は上部・高さ72px以内に収め、入力欄を遮らない。


## 失敗時の確定境界（v0.0.0.5）

会話へのメッセージ追加は複製で準備し、保存成功後にメモリ上の会話を更新・通知する。新規会話も保存成功後に登録する。Rendererは新しいuserレコードまたは成功応答を受け取るまで下書きを保持し、入力の世代番号で後続編集を保護する。LLM文脈は失敗ターン単位で除外し、連続assistant記録は結合する。

登録処理にはAbortSignalを伝え、フォルダ許可の保存前に中断を検査する。履歴削除のworker完了後にも中断を確認し、ライブDBへの削除を止める。バックアップ内の既完了処理は巻き戻さない。VRM登録失敗時はアプリ所有の未登録コピーを削除し、削除失敗は通知する。GGUFの再利用判定には全分割ファイルのサイズ・更新時刻等を含める。
