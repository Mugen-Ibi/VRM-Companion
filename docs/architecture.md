...
表示状態の実装名は`idle / thinking / responding / working / success / error / canceled / attention`。利用可能な`blink`、`happy`、`relaxed`の表情と胸ボーンの軽い待機動作を使い、視線追従は現時点で実装していない。表情のないモデルにも固定モデル固有の名称を要求せず、状態はパネルのテキストでも伝える。特定の衣装・性別・キャラクター設定を前提とせず、LLMへ毎フレームの制御を任せない。VRMに歩行や特定の仕草が付属すると仮定せず、追加動作は後続版のモーション機能として扱う。

切替時は古いモデルのgeometry・material・textureなどを解放する。表情とまばたきの競合を調整し、非表示時は描画を停止、復帰時は経過時間を制限して物理の飛びを抑える。

## 5. llama.cppとの接続

v0.1アプリは外部起動のllama-serverへ接続する。アプリ自身によるLLMバイナリ・モデルの同梱、ダウンロード、GPUバックエンドの自動セットアップは行わない。利用者から今回の導入を許可されたため、開発・セットアップ用の`setup-llama.ps1`と`start-llama.ps1`を別途用意している。明示実行されたスクリプトによる準備と、常駐アプリの動作は区別する。

実接続の検証にはユーザー提供の`Qwen3.5-9B-Q4_K_M.gguf`と既存llama.cpp b9843（`127.0.0.1:8080`）を使用した。公式b10964（0.4.1対応）は`.local/llama/b10964/`へ並置し、CPU・8081で検証中。既存サーバーやモデルを置換しておらず、アプリの既定8080も変更していない。取得資産とSHA-256は`manifest.json`へ記録する。採用済み環境と検証候補の結果は[検証記録](verification.md)で区別する。
