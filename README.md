...
Windowsコード署名証明書の準備が完了するまで、バイナリ配布を一時停止しています。過去の未署名Betaリリースは削除せずドラフトへ戻しました。承認済み設計のv0.1は後続の到達目標であり、現時点で全受け入れ条件の完了を宣言するものではありません。
...
署名済み配布ZIPを展開し、フォルダ内の `VRM-Companion.exe` を起動します。未署名のローカル検証ビルドは配布しないでください。旧版が動作中の場合はトレイから終了してから起動してください。実行ファイルだけ移動せず、フォルダ全体を保持してください。Node.jsやUnityは実行時に不要です。
